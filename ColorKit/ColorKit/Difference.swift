...
extension ColorKitColor {
    public enum ColorDifferenceResult: Comparable {
        ...
        public static func < (
            lhs: ColorKitColor.ColorDifferenceResult,
            rhs: ColorKitColor.ColorDifferenceResult
        ) -> Bool {
    ...
    public enum DeltaEFormula {
        /// The euclidean algorithm is the simplest and fastest one, but will yield results that are
        /// unexpected to the human eye. Especially in the green range.
        ...
        /// The `CIE94` algorithm is an improvement to the `CIE76`, especially for the saturated
        /// regions. It's marginally slower than `CIE76`.
        case CIE94
    ...
    /// Computes the difference between the passed in `ColorKitColor` instance.
    ///
    /// - Parameters:
    ///   - color: The color to compare this instance to.
    ///   - formula: The algorithm to use to make the comparaison.
    /// - Returns: The different between the passed in `ColorKitColor` instance and this instance.
    public func difference(
        from color: ColorKitColor,
        using formula: DeltaEFormula = .CIE94
    ) -> ColorDifferenceResult {
        switch formula {
        case .euclidean:
            let differenceValue = sqrt(pow(red255 - color.red255, 2) + pow(
                green255 - color.green255,
                2
            ) + pow(blue255 - color.blue255, 2))
        ...
        case .CIE76:
            let differenceValue = sqrt(pow(color.L - L, 2) + pow(color.a - a, 2) + pow(
                color.b - b,
                2
            ))
            let roundedDifferenceValue = differenceValue.rounded(.toNearestOrEven, precision: 100)
            return ColorDifferenceResult(value: roundedDifferenceValue)
        case .CIE94:
            let differenceValue = ColorKitColor.deltaECIE94(lhs: self, rhs: color)
            let roundedDifferenceValue = differenceValue.rounded(.toNearestOrEven, precision: 100)
            return ColorDifferenceResult(value: roundedDifferenceValue)
        default:
    ...
    private static func deltaECIE94(lhs: ColorKitColor, rhs: ColorKitColor) -> CGFloat {
        let kL: CGFloat = 1.0
        let kC: CGFloat = 1.0
        let kH: CGFloat = 1.0
