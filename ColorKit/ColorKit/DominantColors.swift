...
private typealias ColorKitImage = UIImage
#else
import AppKit

private typealias ColorKitImage = NSImage
#endif
import CoreImage
...
public class ColorFrequency: CustomStringConvertible {
    ...
    public let color: ColorKitColor
    ...
    init(color: ColorKitColor, count: CGFloat) {
        frequency = count
        self.color = color
    }
}

extension ColorKitImage {
    ...
    public enum DominantColorQuality {
        ...
        /// Returns a new size (with the same aspect ratio) that takes into account the quality to
        /// match.
        /// For example with a `.low` quality, the returned size will be much smaller.
        /// On the opposite, with a `.best` quality, the returned size will be identical to the
        /// original size.
    ...
    /// This is not the absolute dominent colors, but instead colors that are similar are groupped
    /// together.
    /// This avoids having to deal with many shades of the same colors, which are frequent when
    /// dealing with compression artifacts (jpeg etc.).
    /// - Parameters:
    ///   - quality: The quality used to determine the dominant colors. A higher quality will yield
    /// more accurate results, but will be slower.
    ///   - algorithm: The algorithm used to determine the dominant colors. When using a k-means
    /// algorithm (`kMeansClustering`), a `CIKMeans` CIFilter isused. Unfortunately this filter
    /// doesn't work on the simulator.
    /// - Returns: The dominant colors as array of `UIColor` instances. When using the `.iterative`
    /// algorithm, this array is ordered where the first color is the most dominant one.
    public func dominantColors(
        with quality: DominantColorQuality = .fair,
        algorithm: DominantColorAlgorithm = .iterative
    ) throws -> [ColorKitColor] {
        switch algorithm {
        case .iterative:
            let dominantColorFrequencies = try dominantColorFrequencies(with: quality)
            let dominantColors = dominantColorFrequencies.map { colorFrequency -> ColorKitColor in
    ...
    /// This is not the absolute dominent colors, but instead colors that are similar are groupped
    /// together.
    /// This avoid having to deal with many shades of the same colors, which are frequent when
    /// dealing with compression artifacts (jpeg etc.).
    /// - Parameters:
    ///   - quality: The quality used to determine the dominant colors. A higher quality will yield
    /// more accurate results, but will be slower.
    /// - Returns: The dominant colors as an ordered array of `ColorFrequency` instances, where the
    /// first element is the most common one. The frequency is represented as a percentage ranging
    /// from 0 to 1.
    public func dominantColorFrequencies(with quality: DominantColorQuality = .fair) throws
        -> [ColorFrequency]
    {
        // ------
        ...
        for colorFrequency in colorFrequencies {
            ...
            for dominantColor in dominantColors {
                let differenceScore = colorFrequency.color.difference(
                    from: dominantColor.color,
                    using: .CIE76
                ).associatedValue
        ...
        // ------
        // Step 8: Sort again on frequencies because the order may have changed because we combined
        // colors.
        // ------
        ...
        dominantColors = dominantColors.map { colorFrequency -> ColorFrequency in
            let percentage = (100.0 / (dominantColorsTotalCount / colorFrequency.frequency) / 100.0)
                .rounded(
                    .up,
                    precision: 100
                )
    ...
    private func kMeansClustering(with quality: DominantColorQuality) throws -> [ColorKitColor] {
        #if os(iOS)
        ...
        var dominantColors = [ColorKitColor]()

        for i in 0..<clusterCount {
            let color = ColorKitColor(
                red: CGFloat(bitmap[i * 4 + 0]) / 255.0,
                green: CGFloat(bitmap[i * 4 + 1]) / 255.0,
                blue: CGFloat(bitmap[i * 4 + 2]) / 255.0,
                alpha: CGFloat(bitmap[i * 4 + 3]) / 255.0
            )
